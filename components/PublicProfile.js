...
export default function PublicProfile(profileId) {
  ...
  if (loading) return <p className="text-center text-gray-400 mt-20">Laster profil…</p>;
  if (error) return <p className="text-center text-red-400 mt-20">{error}</p>;
  if (!profile || !loggedInUser) return <p className="text-center text-red-400 mt-20">Ingen data funnet.</p>;
      ...
      <div className="max-w-4xl mx-auto bg-neutral-900 rounded-xl shadow-xl p-6 space-y-6 relative">
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="text-center md:text-left">
            ...
            <p className="text-pink-400 italic">{profile.relationshipStatus || "Udefinert status"}</p>
            <p className="text-gray-400">{age} år • {profile.gender}</p>
        ...
        {profile.bio && (
          <div className="bg-neutral-800 p-4 rounded-lg border border-gray-700">
            ...
            <p className="mt-2 text-gray-300 whitespace-pre-line">{profile.bio}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Detail label="Høyde" value={profile.height ? `${profile.height} cm` : ""} />
          <Detail label="Har barn" value={profile.hasChildren ? "Ja" : "Nei"} />
          <Detail label="Vil ha barn" value={profile.wantsChildren ? "Ja" : "Nei"} />
          <Detail label="Røyker" value={profile.smoking} />
          <Detail label="Drikker" value={profile.drinking} />
          <Detail label="Villig til å flytte" value={profile.willingToRelocate ? "Ja" : "Nei"} />
        </div>

        {/* Looking For */}
        <div>
          <h2 className="text-xl font-semibold text-pink-500">Hva jeg ser etter</h2>
          <p className="mt-2 text-gray-300">{profile.lookingFor || "Ikke spesifisert ennå."}</p>
        </div>

        {/* Tags */}
        {profile.tags?.length > 0 && (
          <div>
            <h2 className="text-xl font-semibold text-pink-500">Mine hashtags</h2>
